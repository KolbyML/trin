...
use super::{
    ...
    types::{get_era_type, BlockWithRecoveredSenders, EraType, ProcessedEra},
};
...
impl EraManager {
    ...
    async fn fetch_era_file(&mut self, block_number: u64) -> anyhow::Result<ProcessedEra> {
        ...
        let next_block_number = current_era.first_block_number + current_era.block_count;
...
mod tests {
    ...
    #[traced_test]
    #[tokio::test]
    async fn test_fetching_era_file() {
    ...
    #[traced_test]
    #[tokio::test]
    async fn test_era_manager() {
        let block_number = 1_908_000;
        let mut era_manager = EraManager::new()
            .await
            .expect("to be able to create era manager");
        let block = era_manager
            .get_block_by_number(block_number)
            .await
            .expect("to be able to get block");
