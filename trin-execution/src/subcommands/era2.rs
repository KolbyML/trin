...
impl StateExporter {
    ...
    pub fn export_state(&mut self, header: Header) -> Result<(), Error> {
        ...
        })) = leaf_iterator.next()
        {
            ...
            let size = era2.append_account(&AccountTuple {
                address_hash: AddressHashEntry {
                    address_hash: account_hash,
                },
                account_state: AccountStateEntry {
                    account_state: (&account_state.clone()).into(),
                },
                bytecode: BytecodeEntry {
            ...
            if size > 1024 * 1024 * 50 {
                error!(
                    "Why is this account so big: account_hash: {:?} account_state: {:?} bytecode: {:?} storage: {:?} {:?}",
                    account_hash,
                    alloy_rlp::encode(account_state).len(),
                    bytecode,
                    storage.len(),
                    alloy_rlp::encode(storage).len()
                );
            }
...
impl StateImporter {
    ...
    pub fn import_state(&mut self) -> Result<(), Error> {
        ...
        while let Ok(account_tuple) = era2.next_account() {
            ...
            {
                account_trie.insert(storage_index_hash.as_slice(), &alloy_rlp::encode(value))?;
            }
