            ...
            if let Some(beacon_api_endpoint) = trin_execution_config.beacon_api_endpoint.clone() {
                let bridge_channel = if trin_execution_config.bridge_diffs {
                    ...
                    let bridge_config = BridgeConfig {
                        filter_clients: vec![ClientType::Ultralight],
                        bridge_id: trin_execution_config.bridge_id,
                        enr_offer_limit: 8,
                        ..Default::default()
                    };
            ...
            tokio::select! {
                ...
                Some(command) = self.command_rx.recv() => {
                    match command {
                        ...
                        EngineCommand::NewPayloadV2(_new_payload_message) => todo!(),
                        EngineCommand::NewPayloadV3(_new_payload_message) => todo!(),
                        EngineCommand::GetPayloadV1(_new_payload_message) => todo!(),
                        EngineCommand::GetPayloadV2(_new_payload_message) => todo!(),
                        EngineCommand::GetPayloadV3(_new_payload_message) => todo!(),
                    }
                }
            }
        }
