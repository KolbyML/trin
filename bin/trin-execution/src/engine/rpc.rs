...
use alloy::{
    ...
    rpc::types::{
        engine::{
            ClientCode, ClientVersionV1, ExecutionPayloadBodiesV1, ExecutionPayloadInputV2,
...
impl EngineApiServer for EngineRPCServer {
    ...
    async fn get_payload_v4(&self, _payload_id: PayloadId) -> RpcResult<ExecutionPayloadV3> {
        Err(EngineApiError::ServerError(
            "Trin Execution doesn't support block building for the time being.".to_string(),
        )
        .into())
    ...
    async fn new_payload_v1(&self, payload: ExecutionPayloadV1) -> RpcResult<PayloadStatus> {
        ...
        if block_hash != processed_block.header.hash_slow() {
    ...
    async fn new_payload_v3(
        ...
        // let _block_hash = _payload.payload_inner.payload_inner.block_hash;

        // let processed_block =
        //     ExecutionPayloadV3WithBeaconBlockHash::new(_payload, _parent_beacon_block_root)
        ...
        // if _block_hash != processed_block.header.hash() {
        //     return Err(EngineApiError::ServerError(format!(
        //         "Block hash mismatch: expected {_block_hash} but got {}",
        ...
        // let command =
        //     EngineCommand::new_payload(processed_block, Some(_versioned_hashes), command_tx);
