...
// <network-name>-<block-number>-<short-state-root>.Era2Old
//
// Era2Old := Version | CompressedHeader | account*
// account :=  CompressedAddressHash | CompressedAccountState | CompressedBytecode |
// CompressedStorage
// -----
...
// CompressedAccountState = { type: 0x05,   data: snappyFramed(rlp(nonce, balance, storage_root,
//  code_hash)) }
// CompressedBytecode     = { type: 0x06,   data: snappyFramed(raw_bytecode) }
// CompressedStorage      = { type: 0x07,   data: snappyFramed(rlp(Vec<StorageItem {
// storage_index_hash, value }>)) }

/// Represents an Era2Old `Era2Old` state snapshot.
...
pub struct Era2Old {
    ...
    /// e2store_stream, manages the interactions between the Era2Old state snapshot
...
impl Era2Old {
    ...
    pub fn create(era2_path: PathBuf, header: Header) -> anyhow::Result<Self> {
        ...
        if era2_path.exists() {
            panic!("era2_path file already exists: {:?}", era2_path);
        }
    ...
    pub fn append_account(&mut self, account: &AccountTuple) -> anyhow::Result<usize> {
